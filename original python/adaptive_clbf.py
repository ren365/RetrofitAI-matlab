...
class AdaptiveClbf(object):
	...
	def get_control_safe(self,z,z_ref,z_ref_dot,dt,obs=None,train=False,use_model=False,add_data=True,check_model=True,use_qp=True):
		assert z.shape[0] == self.xdim + 1
		assert z_ref_dot.shape[0] == self.xdim + 1
		assert z_ref.shape[0] == self.xdim + 1
		...
		if use_qp:
			...
			self.mu_new = self.qpsolve.solve(self.z,self.z_ref,mu_new,sigDelta)  + mu_new
	...
	def get_control_safe_debug(self,z,z_ref,z_ref_dot,dt,obs=None,train=False,use_model=False,add_data=True,check_model=True,use_qp=True):
		assert z.shape[0] == self.xdim + 1
		assert z_ref_dot.shape[0] == self.xdim + 1
		assert z_ref.shape[0] == self.xdim + 1
