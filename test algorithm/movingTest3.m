...
for i = 1:N-2
	...
	adaptive_clbf = adaptive_clbf.update_barrier_locations(barriers_locations(i,1:2:end-1), ...
		barriers_locations(i,2:2:end),mean(barriers_radius)+0.2);
	...
	current_distance = nearest_distance(barriers_locations(i+1,:),x(:,i), ...
									barriers_radius-0.2,barriers_numbers); % barriers_radius-0.2 = actual circle
...
if true
	...
	for i = 1:N-1
		...
		circles(barrier_x,barrier_y,radius-0.2,'facecolor','red','edgecolor','red');
		yline(-1.5)
		yline(-4.5)
		yline(1.5)
		yline(4.5)
		legend({'reference path','paper''s method','detected cars','actual cars'},'location','northeastoutside');
