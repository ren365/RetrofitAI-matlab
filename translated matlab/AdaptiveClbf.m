classdef AdaptiveClbf
	...
	function result = saturate(obj,u,ulim)
		u0=u[0];
		u0(u0>ulim[0][1])=ulim[0][1];
		u0(u0<ulim[0][0])=ulim[0][0];
		
		u1=u[1];
		u1(u1>ulim[1][1])=ulim[1][1];
		u1(u1<ulim[1][0])=ulim[1][0];
	...
	% untested
	function [obj,controls] = get_control(obj,z_ref,z_ref_dot,dt,obs,train,use_model,add_data,check_model,use_qp)
		...
		if use_qp:
			obj.mu_qp = obj.qpsolve.solve(obj.z,obj.z_ref,mu_d,sigDelta);
